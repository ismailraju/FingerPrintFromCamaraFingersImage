...
//            Mat imageColor = bytesToMat(data);
        ...
        p.matToJpg(image, "COLOR_BGR2GRAY");
        ...
        p.matToJpg(equalized, "equalized");
        ...
        p.matToJpg(floated, "floated");

        Mat skeleton = getSkeletonImage(floated, rows, cols);
        p.matToJpg(skeleton, "skeleton");

        incrementProgress();

        skeleton = thinning(skeleton);
        p.matToJpg(skeleton, "thinning");
        incrementProgress();

        Mat skeleton_with_keypoints = detectMinutiae(skeleton, 1);
        p.matToJpg(skeleton_with_keypoints, "detectMinutiae");
        incrementProgress();

        return skeleton_with_keypoints;
...
//        });
    ...
    private int neighbourCount(Mat skeleton, int row, int col) {
        int cn = 0;
        if (skeleton.get(row - 1, col - 1)[0] != 0) {
            cn++;
        }
        if (skeleton.get(row - 1, col)[0] != 0) {
            cn++;
        }
        if (skeleton.get(row - 1, col + 1)[0] != 0) {
            cn++;
        }
        if (skeleton.get(row, col - 1)[0] != 0) {
            cn++;
        }
        if (skeleton.get(row, col + 1)[0] != 0) {
            cn++;
...
//        Log.d(TAG, String.format("Filtered %s Keypoints", filterCount));
        System.out.println("Filtered %s Keypoints" + filterCount);
    ...
    /**
     * Match using the ratio test and RANSAC. Returns the ratio matches and the
     * total keypoints
     */
...
//    @NonNull
    ...
    /**
     * calculate ridge segment by doing block process for the given image using
    ...
    /**
     * Enhance the image after ridge filter. Apply mask, binary threshold,
     * thinning, ..., etc.
     */
...
//        if (BuildConfig.DEBUG && !paddedMask.size().equals(source.size())) {
        if (!paddedMask.size().equals(source.size())) {
    ...
    static Mat preprocess(Mat frame, int width, int height) {
